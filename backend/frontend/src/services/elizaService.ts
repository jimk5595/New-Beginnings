export interface ElizaResponse {
  success: boolean;
  message: string;
  assigned_to?: string;
  raw?: any;
}

export async function sendToEliza(userMessage: string, attachments: string[] = []): Promise<ElizaResponse> {
  try {
    const response = await fetch("http://127.0.0.1:8000/api/eliza/task", {
      method: "POST",
      headers: {
        "Content-Type": "application/json"
      },
      body: JSON.stringify({ 
        task_text: userMessage,
        attachments: attachments 
      })
    });
    if (!response.ok) {
      const errorText = await response.text();
      return {
        success: false,
        message: `Error ${response.status}: ${errorText || "Backend returned an error status."}`
      };
    }
    const data = await response.json();
    
    // Support integration dictionaries and delegate_to mapping
    const orch = data.orchestration || {};
    const assignedTo = data.assigned_to || orch.assigned_to || "eliza";
    const message = data.response || "No response received.";

    return {
      success: true,
      message: message,
      assigned_to: assignedTo,
      raw: data
    };
  } catch (error: any) {
    return {
      success: false,
      message: `Connection failed: ${error.message || "Failed to reach backend."}`
    };
  }
}
