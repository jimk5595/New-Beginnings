...
HARD_RULES = (
    ...
    "CALL_LLM_ASYNC IMPORT MANDATE — CRITICAL: If app.py uses `call_llm_async` in ANY route function, you MUST add `from core.llm_client import call_llm_async` as a MODULE-LEVEL import at the top of app.py (directly after the standard imports, before `router = APIRouter()`). DO NOT rely on inline per-function imports — they only cover the function they are written in. Functions that call `call_llm_async()` without this module-level import raise `NameError: name 'call_llm_async' is not defined`, which is silently caught by the route's except block and returns 'Service temporarily unavailable' with all-zero data. Per CALL_LLM_ASYNC IMPORT MANDATE.\n"
